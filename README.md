...
## Запуск проекта
**После клонирования репозитория, настройки виртуального окружения, выполняем следующие команды:**
- **устанавливаем необходимые зависимости если этого еще не сделали `poetry install`;**
- **применяем миграции (перед применением миграций не забудьте создать и настроить БД) `python manage.py migrate`;**

**Далее заполняем БД первичными данными и создать суперпользователя(администратора сайта) используя следующие команды:**
